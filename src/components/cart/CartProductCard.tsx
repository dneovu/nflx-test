...
const CartProductCard = ({ id, title, price, imgSrc, quantity }: CartItem) => {
    ...
    <article className="flex max-w-158 flex-col gap-4 rounded-2xl bg-white p-6 shadow-md sm:min-w-158">
      ...
      <div className="flex items-center justify-between pl-3.5">
        <div className="flex gap-6">
          <QuantityButton quantity={quantity} action="decrease" itemId={id} />

          <div className="flex w-3 justify-center">
            <span>{quantity}</span>
          </div>
          <QuantityButton quantity={quantity} action="increase" itemId={id} />
        </div>
